...
__device__ double slope(double *q, int i, int nv){
    double dlft, drgt, dcen, dsgn, dlim;
    dlft=dev_sl_type*(q[nv*(blockDim.x+2)+i  ]-q[nv*(blockDim.x+2)+i-1]);
    drgt=dev_sl_type*(q[nv*(blockDim.x+2)+i+1]-q[nv*(blockDim.x+2)+i  ]);
...
__global__ void trace(double *qxm, double *qxp, double *q, int np, int nt, double dtdx){
    ...
    for(k=0;k<dev_nvar;++k){
      stride=0;
      while(threadIdx.x+stride<blockDim.x+2){
        qLoc[k*(blockDim.x+2)+threadIdx.x+stride]=q[(k*nt+j)*(np+2)+i+stride];
        stride+=blockDim.x;
      }
    }
    ...
    if(i<np&&j<nt){
      ...
      r=qLoc[ID*(blockDim.x+2)+threadIdx.x+1];
      u=qLoc[IU*(blockDim.x+2)+threadIdx.x+1];
      v=qLoc[IV*(blockDim.x+2)+threadIdx.x+1];
      p=qLoc[IP*(blockDim.x+2)+threadIdx.x+1];
      ...
      //calculate slopes
      dr=slope(qLoc,threadIdx.x+1,ID);
      du=slope(qLoc,threadIdx.x+1,IU);
      dv=slope(qLoc,threadIdx.x+1,IV);
      dp=slope(qLoc,threadIdx.x+1,IP);
...
__global__ void crDispBuff(uchar4 *buff, double *u, int dispVar, float datN, float datX){
     uint ind=threadIdx.x+blockDim.x*blockIdx.x;
     float dispVal=0.0;

    if(ind<dev_nx*dev_ny){
      dispVal=(u[(dispVar*dev_nx*dev_ny)+ind]-datN)/(datX-datN);
      dispVal=(dispVal<0.0)?0.0:dispVal;
      dispVal=(dispVal>1.0)?1.0:dispVal;
      //convert to color
      buff[ind].x=0x00+0xff*dispVal;
      buff[ind].y=0x88-0xff*fabs(0.5-dispVal);
      buff[ind].z=0xff-0xff*dispVal;
      buff[ind].w=0xff;
    }
}
